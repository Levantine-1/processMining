terraform {
  backend "s3" {
    ...
    key          = "processMining/terraform.tfstate"
    region       = "us-west-2"
    use_lockfile = true
  }
}
...
# Still needed here (unlike portfolio/DataGateway, which dropped this
# entirely): the aws.delegate provider in route_53_delegate_records.tf
# manages a real, live record ("processmining.levantine.io") and
# genuinely needs Vault for its credentials. Only the *default* AWS
# provider below moves to OIDC.
provider "vault" {
